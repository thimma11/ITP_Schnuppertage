...
router.get('/:dep_id/:loc_id', (req, res) => {
    ...
    if(req.query.count == 'true') {
        connection.query(`select count(g.id) as count from timetables t join groups g on t.departments_id=g.department_id and t.locations_id=g.location_id group by g.department_id, g.location_id;`, [dep_id, loc_id], function (error, results, fields) {
            if (error) throw error;
            res.json(results);
        });
    }
